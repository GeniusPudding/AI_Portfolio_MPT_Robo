...
export default {
  ...
  computed: {
    testMode() {
      return location.hostname != "event.franklin.com.tw" && location.hostname != "www.franklin.com.tw";
    },
    localMode() {
    ...
    getScore() {
      console.log('getScore this:',this)
    ...
    fundLength() {
      return this.res.rp_list ? this.res.rp_list.length : 0;
    },
  },
  beforeMount() {
  ...
  methods: {
    ...
    tryAgain() {
      ...
      this.$nextTick(() => {
        this.$scrollTo("#questionnaire");
        this.$refs.notSuitable.toggle = false;
      });
    },
    toggle() {
      ...
      await new Promise((resolve) => {
        setTimeout(() => {
          if (this.getScore == 99) {
            ...
            this.complete = true;
            this.$refs.notSuitable.toggle = true;
    ...
    serieChart(resolve) {
      ...
      // 暫存所有類別資料
      tempSeries = this.res.rp_list.map((e) => {
        return { cate: e.FundInfo.FundCategory, ratio: e.InvestRatio };
      });

      // 整理類別資料
      tempSeries.forEach((obj, key) => {
        // 從暫存判斷重複類別
        if (!tempCate.includes(obj.cate)) {
          tempCate.push(obj.cate);
          let tempIndex = tempCate.indexOf(obj.cate) < 0 ? 0 : tempCate.indexOf(obj.cate);
          tempData[tempIndex] = { cate: obj.cate, ratio: obj.ratio };
        } else {
          // 重複類別取出比例加總
          let index = tempData.filter((e) => e.cate == obj.cate);
          let oldRatio = tempData[tempData.indexOf(index[0])].ratio;
          let newRatio = oldRatio + obj.ratio;
          tempData[tempData.indexOf(index[0])].ratio = newRatio;
        }
      });

      // console.log("tempData:", tempData);

      series = tempData.map((item) => [item.cate, item.ratio]);
